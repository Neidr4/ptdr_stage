...
class movebaseHandler:
    ...
    def callback_movebaseHandler(self, msg):
        ...
        # print("callback self.last_received_pose.pose.position.x: " + str(self.last_received_pose.pose.position.x) )
        # print("callback self.last_received_pose.pose.position.y: " + str(self.last_received_pose.pose.position.y) )
    ...
    def pose_checker(self):
        ...
        offset_x = 0.0 #-0.55
        offset_y = 0.0 #-0.4
        pose_checker_x = self.last_received_pose.pose.position.x
        pose_checker_y = self.last_received_pose.pose.position.y
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        now = rospy.get_time()
        if((now - movebaseHandler0.last_run) > freq):
            print("now = " + str(now))
            print("time diff = " + str(now - movebaseHandler0.last_run))
            ...
            movebaseHandler0.last_run += freq 
            # rospy.sleep(pause)

        now = rospy.get_time()
        if((now - movebaseHandler1.last_run) > freq):
        ...
        now = rospy.get_time()
        if((now - movebaseHandler2.last_run) > freq):
        ...
        now = rospy.get_time()
        if((now - previous_map_update) > freq*2):
